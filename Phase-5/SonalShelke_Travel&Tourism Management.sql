...
-- REAL-WORLD BASED SCENARIO
-- 1. Global Market Expansion Analysis
-- A company wants to expand into high-population Asian countries where English is commonly used.
SELECT country_name, population, currency
FROM Countries
WHERE region = 'Asia'
  AND population > (
...
-- 2. Population Ranking by Region (Government Report)
-- A government dashboard ranks countries within each region based on population.

SELECT country_name, region, population,
       RANK() OVER (PARTITION BY region ORDER BY population DESC) AS region_rank
FROM Countries;

-- 3. Identify Over-Populated Countries (Policy Planning)
-- UN wants countries whose population is higher than the regional average.
...
-- 4. Regional Population Summary View (Analytics Team)
-- Create a reusable view for regional population insights.
CREATE VIEW Region_Population_Summary AS
SELECT region,
...
-- Use the view
SELECT * FROM Region_Population_Summary
ORDER BY total_population DESC;

-- 5. Secure Country Data Update 
-- Safely update population during a census with rollback protection.
...
-- REAL-WORLD BASED SCENARIO
-- 1. Capital Cities with High Population (Government Planning)
-- Identify capital cities with population above the average capital population globally.

SELECT ci.city_name, co.country_name, ci.population
FROM Cities ci
JOIN Countries co ON ci.country_id = co.country_id
...
-- 2. Urban Density Analysis (Smart City Project)
-- Find cities with very high population density (people per km²).
...
-- Urban development ministry wants to rank cities inside each country.

SELECT ci.city_name,
       co.country_name,
       ci.population,
       RANK() OVER (PARTITION BY ci.country_id ORDER BY ci.population DESC) AS city_rank
FROM Cities ci
JOIN Countries co ON ci.country_id = co.country_id;


-- 4. Countries Where Non-Capital Cities Are Larger Than Capitals
-- Policy analysts want countries where economic hubs are not capitals.

SELECT DISTINCT co.country_name
FROM Countries co
JOIN Cities capital ON co.country_id = capital.country_id AND capital.is_capital = TRUE
JOIN Cities non_capital ON co.country_id = non_capital.country_id AND non_capital.is_capital = FALSE
WHERE non_capital.population > capital.population;

-- 5. Airport Infrastructure Coverage Report
-- Aviation authority wants cities with airports in countries having population > 100M.

SELECT ci.city_name, ci.airport_code, co.country_name
FROM Cities ci
JOIN Countries co ON ci.country_id = co.country_id
WHERE ci.airport_code IS NOT NULL
  AND co.population > 100000000
...
-- 1. Top-Rated Tourist Destinations by Country (Tourism Board)
-- Tourism ministry wants the top 3 highest-rated destinations in each country.
SELECT country_name, destination_name, rating
FROM (
    SELECT co.country_name,
...
-- 2. High-Value Paid Attractions (Revenue Optimization)
-- Identify attractions with high ratings (≥4.7) that charge an entry fee, to focus on premium tourism.
...
WHERE d.rating >= 4.7
  AND d.entry_fee > 0
ORDER BY d.entry_fee DESC;


-- 3. Cities with More Than One Major Tourist Attraction
-- Travel platforms want to promote cities having multiple attractions.

SELECT ci.city_name,
       co.country_name,
       COUNT(d.destination_id) AS total_attractions
FROM Destinations d
JOIN Cities ci ON d.city_id = ci.city_id
JOIN Countries co ON ci.country_id = co.country_id
GROUP BY ci.city_name, co.country_name
HAVING COUNT(d.destination_id) > 1
ORDER BY total_attractions DESC;

-- 4. Free vs Paid Attractions Analysis 
-- Analyze how free attractions compare to paid ones in terms of ratings.
...
-- 5. Countries with World-Class Historical Sites
-- UNESCO-style report: countries having historical destinations rated above their country’s average.
...
WHERE d.category = 'Historical'
  AND d.rating > (
      ...
      WHERE c2.country_id = co.country_id
  );
...
-- REAL-WORLD BASED EXAMPLES
-- 1. Premium Hotels Near Top-Rated Tourist Destinations
-- Travel agencies want 5-star hotels located near highly rated destinations (≥ 4.7).
...
-- 2. Hotel Price Comparison by Country (Market Analysis)
-- Hospitality analysts want the average hotel price per country to compare markets.

SELECT co.country_name,
       ROUND(AVG(h.price_per_night), 2) AS avg_price_per_night
FROM Hotels h
JOIN Destinations d ON h.destination_id = d.destination_id
JOIN Cities ci ON d.city_id = ci.city_id
JOIN Countries co ON ci.country_id = co.country_id
...
-- 3. Identify High-Demand Hotels (Inventory Management)
-- Operations team wants hotels with low room availability in popular destinations.
...
JOIN Destinations d ON h.destination_id = d.destination_id
JOIN Cities ci ON d.city_id = ci.city_id
WHERE h.rooms_available < 40
  AND d.rating >= 4.5
ORDER BY h.rooms_available ASC;

-- 4. Rank Hotels by Price Within Each City
-- Booking platforms rank hotels within a city to show cheapest → costliest options.
...
-- 5. Countries Offering Luxury Tourism Experience
-- Tourism board wants countries that have at least one luxury hotel (5⭐) and a major attraction.
...
-- REAL-WORLD EXAMPLES
-- 1. Popular International Routes with Affordable Pricing
-- A travel app wants to highlight international flights priced below the overall average fare.
...
SELECT f.flight_number,
       ...
       f.price
FROM Flights f
JOIN Cities dep ON f.departure_city_id = dep.city_id
JOIN Cities arr ON f.arrival_city_id = arr.city_id
JOIN Countries c1 ON dep.country_id = c1.country_id
JOIN Countries c2 ON arr.country_id = c2.country_id
WHERE c1.country_id <> c2.country_id
  AND f.price < (SELECT AVG(price) FROM Flights)
ORDER BY f.price ASC;

--  2. Airline Revenue Potential Analysis
-- Airline management wants to estimate potential revenue per airline

SELECT airline,
       SUM(price * seats_available) AS potential_revenue
FROM Flights
GROUP BY airline
ORDER BY potential_revenue DESC;

-- 3. Busiest Travel Cities 
-- Aviation authority wants cities with high flight traffic (departures + arrivals).

SELECT ci.city_name,
       COUNT(f.flight_id) AS total_flights
FROM Cities ci
JOIN Flights f
  ON ci.city_id = f.departure_city_id
  OR ci.city_id = f.arrival_city_id
GROUP BY ci.city_name
ORDER BY total_flights DESC;

-- 4. Rank Flights by Price Within Each Airline
-- Booking platforms rank flights from cheapest to costliest per airline.
...
-- 5. Long-Haul Flights with Low Seat Availability (Operations Alert)
-- Operations team wants flights longer than 6 hours with low seat availability.

SELECT f.flight_number,
       dep.city_name AS from_city,
       arr.city_name AS to_city,
       TIMESTAMPDIFF(HOUR, f.departure_time, f.arrival_time) AS flight_hours,
       f.seats_available
FROM Flights f
JOIN Cities dep ON f.departure_city_id = dep.city_id
JOIN Cities arr ON f.arrival_city_id = arr.city_id
WHERE TIMESTAMPDIFF(HOUR, f.departure_time, f.arrival_time) >= 6
  AND f.seats_available < 90
...
-- REAL-WORLD EXAMPLES
-- 1. Top Airlines by Fleet Size (Industry Benchmarking)
-- Aviation analysts want to identify global airlines with the largest fleets.
...
-- 2. Airline Presence by Country 
-- Governments want to see how many airlines operate from each country.
...
-- 3. Airlines Operating High-Volume International Flights
-- Travel platforms want airlines that operate international routes with high seat availability.

SELECT DISTINCT a.airline_name,
       c1.country_name AS departure_country,
       c2.country_name AS arrival_country
FROM Flights f
JOIN Airlines a ON f.airline = a.airline_name
JOIN Cities dep ON f.departure_city_id = dep.city_id
JOIN Cities arr ON f.arrival_city_id = arr.city_id
JOIN Countries c1 ON dep.country_id = c1.country_id
JOIN Countries c2 ON arr.country_id = c2.country_id
...
-- 4. Airline Revenue Potential vs Fleet Size
-- Management wants to compare fleet size with potential flight revenue.

SELECT a.airline_name,
       a.fleet_size,
       SUM(f.price * f.seats_available) AS potential_revenue
FROM Airlines a
JOIN Flights f ON a.airline_name = f.airline
GROUP BY a.airline_name, a.fleet_size
ORDER BY potential_revenue DESC;

-- 5. Legacy Airlines Still Actively Operating
-- Identify oldest airlines (founded before 1950) that are still running flights.
...
-- REAL-WORLD EXAMPLES
-- 1. Customer Demographics by Country (CRM Analytics)
-- The business wants to understand customer distribution by country and gender.
...
FROM Customers c
JOIN Countries co ON c.country_id = co.country_id
GROUP BY co.country_name, c.gender
ORDER BY total_customers DESC;


-- 2. Identify High-Value Customer Markets (Age-Based Targeting)
-- Marketing targets customers aged 25–40, the most active travel segment.
...
FROM Customers c
JOIN Countries co ON c.country_id = co.country_id
WHERE TIMESTAMPDIFF(YEAR, c.date_of_birth, CURDATE()) BETWEEN 25 AND 40
ORDER BY age;


-- 3. Countries with Strong Female Traveler Base
-- Tourism planners want countries where female travelers exceed male travelers.
...
-- 4. Customers Eligible for International Travel (Compliance Check)
-- Travel agencies need customers who have a passport and valid country mapping.
...
-- 5. Customer Signup Trend by Country (Growth Monitoring)
-- Management monitors customer acquisition trends by country.
...
-- Management wants to know which countries generate the highest booking revenue.
...
-- Identify customers who have spent more than the average booking value.
...
-- 3. Pending Payments Risk Report
-- Finance team wants to track pending payments with upcoming check-ins.
...
WHERE b.payment_status = 'Pending'
  AND b.check_in <= DATE_ADD(CURDATE(), INTERVAL 15 DAY)
ORDER BY b.check_in;


-- 4. Most Popular Travel Routes 
-- Travel planners want to identify most booked flight routes.

SELECT dep.city_name AS from_city,
       arr.city_name AS to_city,
...
-- 5. Booking Value Ranking by Month 
-- Management wants to rank bookings by value each month.
...
-- REAL-WORLD EXAMPLES
-- 1. Daily Successful Payment Collection 
-- Finance team wants to track how much money is successfully collected each day.
...
-- 2. Pending Payments with Customer Details 
-- Identify customers whose payments are still pending so reminders can be sent.
...
-- Business wants to know which payment method customers prefer most.
...
-- 4. Booking vs Payment Mismatch
-- Find bookings where payment amount does not match booking amount.
...
WHERE b.total_amount <> p.amount;


-- 5. Top Spending Customers Based on Successful Payments
-- Identify high-value customers based on total successful payments.
...
-- REAL-WORLD EXAMPLES
-- 1. Top-Rated Destinations Based on Customer Reviews
-- Management wants to identify most loved destinations to promote them.
...
-- 2. Airlines with Highest Average Customer Ratings
-- Evaluate airline service quality based on passenger reviews.
...
-- 3. Customers Who Gave Only 5-Star Reviews 
-- Identify happy customers for loyalty programs or testimonials.

SELECT c.first_name,
       c.last_name,
       COUNT(r.review_id) AS five_star_reviews
FROM Reviews r
JOIN Customers c ON r.customer_id = c.customer_id
WHERE r.rating = 5
GROUP BY c.customer_id;

-- 4. Detect Low-Rated Reviews Requiring Attention
-- Support team wants to act on poor experiences quickly.
...
-- Track how customer satisfaction changes month by month.
...
-- REAL-WORLD EXAMPLES
-- 1. Most Expensive & Premium Travel Packages
-- Management wants to identify high-value premium packages for elite customers.
...
-- 2. Agents Offering the Highest Number of Packages
-- Find top-performing travel agents based on package count.
...
-- 3. Packages Suitable for Large Groups 
-- Sales team wants packages that can handle large group bookings.
...
-- Customers want maximum days at minimum cost.

SELECT package_name,
       duration_days,
       price_per_person,
       ROUND(price_per_person / duration_days, 2) AS price_per_day
FROM Packages
ORDER BY price_per_day ASC;

-- 5. Short-Duration Packages for Weekend Travelers
-- Identify weekend or short-trip packages (high demand segment).
...
-- REAL-WORLD EXAMPLES
-- 1. Day-wise Itinerary for a Selected Travel Package
-- Customer wants to see the full daily plan before booking a package.
...
SELECT p.package_name,
       ...
       pi.price_addon
FROM Packages p
JOIN PackageItems pi ON p.package_id = pi.package_id
WHERE p.package_id = 1
ORDER BY pi.day_no, pi.sequence_no;

-- 2. Packages with Paid Add-On Activities
-- Sales team wants to upsell optional paid experiences.
...
FROM Packages p
JOIN PackageItems pi ON p.package_id = pi.package_id
WHERE pi.price_addon > 0
ORDER BY pi.price_addon DESC;

-- 3. Total Extra Cost a Customer May Pay per Package
-- Finance team wants to calculate hidden or additional costs.

SELECT p.package_name,
       SUM(pi.price_addon) AS total_addon_cost
FROM Packages p
JOIN PackageItems pi ON p.package_id = pi.package_id
GROUP BY p.package_name;

-- 4. Packages That Include Both Hotel Stay and Activities
-- Customers prefer all-inclusive packages (stay + experiences).
...
GROUP BY p.package_name
HAVING 
    SUM(pi.item_type = 'Hotel') > 0
AND SUM(pi.item_type = 'Activity') > 0;

-- 5. Identify Activity-Heavy Packages (Experience-Focused Trips)
-- Marketing team promotes experience-rich travel packages.

SELECT p.package_name,
       COUNT(*) AS total_activities
FROM Packages p
JOIN PackageItems pi ON p.package_id = pi.package_id
WHERE pi.item_type = 'Activity'
GROUP BY p.package_name
...
-- Customer wants to see rooms currently available in a selected hotel.

SELECT room_number, room_type, price_per_night, max_occupancy, floor_no
FROM HotelRooms
WHERE hotel_id = 1
  AND status = 'Available';

-- 2. Get Cheapest Available Room Per Hotel
-- Booking engine needs to display “Starting from ₹X per night”.
...
-- 3. Find Family or Group-Friendly Rooms (High Occupancy)
-- Customer traveling with family wants rooms for 4+ people.

SELECT hotel_id, room_number, room_type, max_occupancy, price_per_night
FROM HotelRooms
WHERE max_occupancy >= 4
  AND status = 'Available';

-- 4. List Non-Refundable Rooms to Show Budget Options
-- System highlights cheaper non-refundable deals.
...
-- 5. Hotel Room Availability Summary (Inventory Dashboard)
-- Hotel manager wants a room availability snapshot.
...
-- REAL-WORLD EXAMPLES
-- 1. Show All Available Seats for a Selected Flight
-- Passenger wants to choose a seat before booking.

SELECT seat_number, seat_class, seat_price, is_window, is_aisle
FROM FlightSeats
WHERE flight_id = 1
  AND status = 'Available'
ORDER BY seat_class, seat_price;

-- 2. Find Cheapest Available Seat per Class on a Flight
-- UI shows “Cheapest Economy / Business seat available”.

SELECT seat_class,
       MIN(seat_price) AS cheapest_price
FROM FlightSeats
WHERE flight_id = 2
  AND status = 'Available'
GROUP BY seat_class;

-- 3. List Window Seats Still Available (High Demand Seats)
-- Passenger prefers window seat.

SELECT seat_number, seat_class, seat_price
FROM FlightSeats
WHERE is_window = TRUE
  AND status = 'Available'
ORDER BY seat_price;

-- 4. Seat Occupancy Summary per Flight (Load Factor View)
-- Airline operations team tracks seat utilization.

SELECT flight_id,
       COUNT(*) AS total_seats,
       SUM(status = 'Booked') AS booked_seats,
       SUM(status = 'Available') AS available_seats
FROM FlightSeats
GROUP BY flight_id;

-- 5. Customers Who Booked Premium Seats (Business / First)
-- Marketing targets high-value customers.
...
FROM FlightSeats fs
JOIN Customers c
  ON fs.booked_by_customer = c.customer_id
WHERE fs.seat_class IN ('Business','First')
...
-- REAL-WORLD EXAMPLES
-- 1. Show All Activities Available at a Selected Destination
-- Traveler opens a destination page and wants to see things to do.

SELECT activity_name, activity_type, duration_minutes, price
FROM Activities
WHERE destination_id = 1
ORDER BY price;

-- 2. Find Free or Budget-Friendly Activities
-- Backpackers look for free or low-cost experiences.

SELECT activity_name, destination_id, activity_type
FROM Activities
WHERE price = 0
ORDER BY duration_minutes DESC;

-- 3. List Adventure Activities Suitable for Kids
-- Family booking wants age-appropriate adventure activities.

SELECT activity_name, duration_minutes, min_age, max_participants
FROM Activities
WHERE activity_type = 'Adventure'
  AND min_age <= 10;

-- 4. Most Popular Group Activities (High Capacity Tours)
-- Tour operator plans group tours & logistics.
...
-- 5. Revenue Potential per Activity
-- Business team estimates maximum earning per activity session.

SELECT activity_name,
       price,
       max_participants,
       (price * max_participants) AS max_revenue_per_slot
FROM Activities
ORDER BY max_revenue_per_slot DESC;
...
-- REAL-WORLD EXAMPLES
-- 1. View All Upcoming Confirmed Activity Bookings
-- Operations team checks confirmed bookings to prepare guides & logistics.

SELECT ab.act_booking_id,
       a.activity_name,
       ab.booking_date,
       ab.participants,
       ab.total_amount
FROM ActivityBookings ab
JOIN Activities a
  ON ab.activity_id = a.activity_id
WHERE ab.status = 'Confirmed'
ORDER BY ab.booking_date;

-- 2. Find Cancelled Activities for Refund Processing
-- Finance team identifies cancelled bookings for refunds.
SELECT ab.act_booking_id,
       a.activity_name,
       c.first_name,
       c.last_name,
...
-- 3. Daily Revenue from Activities
-- Management tracks activity-wise earnings per day.
SELECT booking_date,
       SUM(total_amount) AS daily_activity_revenue
FROM ActivityBookings
WHERE status = 'Confirmed'
GROUP BY booking_date
ORDER BY booking_date;

-- 4. Most Popular Activities by Number of Participants
-- Business team decides which activities to promote more.
SELECT a.activity_name,
       SUM(ab.participants) AS total_participants
FROM ActivityBookings ab
JOIN Activities a
  ON ab.activity_id = a.activity_id
WHERE ab.status = 'Confirmed'
...
-- 5. Customers with High-Value Activity Spend
-- CRM team targets premium customers for upselling.
SELECT c.customer_id,
       c.first_name,
       c.last_name,
       SUM(ab.total_amount) AS total_spent
FROM ActivityBookings ab
JOIN Customers c
  ON ab.customer_id = c.customer_id
WHERE ab.status = 'Confirmed'
GROUP BY c.customer_id, c.first_name, c.last_name
HAVING SUM(ab.total_amount) > 5000
ORDER BY total_spent DESC;
...
-- REAL-WORLD EXAMPLES
-- 1. View All Active (Ongoing or Upcoming) Car Rentals
-- Operations team monitors current & upcoming rentals.
...
WHERE cr.status IN ('Booked','Completed')
ORDER BY cr.start_date;

-- 2. Find Cancelled Rentals for Refund or Re-allocation
-- Fleet team reassigns vehicles from cancelled bookings.
...
-- 3. Most Popular Vehicle Types
-- Business team decides fleet expansion strategy.
SELECT vehicle_type,
       COUNT(*) AS total_rentals
FROM CarRentals
...
-- 4. Revenue Generated by Car Rentals
-- Finance team tracks earnings from rentals.
SELECT SUM(total_cost) AS total_car_rental_revenue
FROM CarRentals
WHERE status = 'Completed';

-- 5. Customers Who Spend the Most on Car Rentals
-- CRM team identifies premium mobility customers.
SELECT c.customer_id,
       c.first_name,
       c.last_name,
       SUM(cr.total_cost) AS total_spent
FROM CarRentals cr
JOIN Customers c
  ON cr.customer_id = c.customer_id
WHERE cr.status <> 'Cancelled'
...
-- REAL-WORLD EXAMPLES
-- 1. List All Active Vehicles Currently in Service
-- Fleet manager checks vehicles ready for allocation.
...
FROM RentalVehicles
WHERE status = 'Active'
ORDER BY year DESC, mileage;

-- 2. Identify Vehicles Due for Maintenance (High Mileage)
-- Maintenance team flags vehicles nearing service limit.
SELECT license_plate,
       brand,
       model,
       mileage
...
-- Management analyzes fuel mix (Petrol / Diesel / Hybrid / Electric).
SELECT fuel_type,
...
-- 4. Track Vehicles Linked to Cancelled Rentals
-- Reassign vehicles from cancelled bookings.
SELECT rv.license_plate,
       rv.brand,
       rv.model,
       cr.status AS rental_status
FROM RentalVehicles rv
JOIN CarRentals cr
  ON rv.rental_id = cr.rental_id
WHERE cr.status = 'Cancelled';

-- 5. New vs Old Fleet Analysis
-- Finance team decides replacement strategy.
SELECT
    CASE
        WHEN year >= 2022 THEN 'New Fleet'
        ELSE 'Old Fleet'
...
-- REAL-WORLD EXAMPLE
-- 1. Find Top-Rated Tour Guides for Premium Customers
-- Assign best guides to VIP or luxury tour bookings.
SELECT guide_name,
       specialization,
       rating,
       years_experience
...
-- 2. Language-Wise Guide Availability
-- Operations team checks available guides by language for international tourists.
SELECT language,
       COUNT(*) AS total_guides
FROM TourGuides
WHERE status = 'Active'
...
-- 3. Identify Highly Experienced Guides (Senior Experts)
-- Use senior guides for complex historical or long-duration tours.
SELECT guide_name,
       years_experience,
       specialization,
       rating
...
-- 4. Specialization-Based Guide Matching
-- Auto-assign guides based on tour theme (e.g., Historical, Adventure, Cultural).
SELECT guide_name,
       language,
       rating
FROM TourGuides
WHERE specialization LIKE '%Historical%'
   OR specialization LIKE '%History%'
ORDER BY rating DESC;

-- 5. Performance Review: Average Rating by Experience Level
-- Management analyzes whether experience improves customer satisfaction.
SELECT
    CASE
        WHEN years_experience < 5 THEN 'Junior'
        WHEN years_experience BETWEEN 5 AND 9 THEN 'Mid-Level'
        ELSE 'Senior'
...
-- REAL-WORLD EXAMPLES
-- 1. Find Currently Assigned Guides with Their Activity Details
-- Operations team checks who is guiding which activity today.
SELECT g.guide_name,
       a.activity_name,
       ga.start_time,
       ga.end_time,
       ga.role
FROM GuideAssignments ga
JOIN TourGuides g ON ga.guide_id = g.guide_id
JOIN Activities a ON ga.activity_id = a.activity_id
WHERE ga.status = 'Assigned';

-- 2. Identify Guides with Cancelled Assignments (Risk & Reliability Check)
-- Management wants to review cancellations caused by weather or other issues.
SELECT g.guide_name,
       a.activity_name,
       ga.start_time,
       ga.notes
...
-- 3. Workload Analysis: Number of Assignments per Guide
-- Avoid overloading guides and balance schedules.
SELECT g.guide_name,
       COUNT(ga.assignment_id) AS total_assignments
FROM TourGuides g
LEFT JOIN GuideAssignments ga
       ON g.guide_id = ga.guide_id
       AND ga.status = 'Assigned'
GROUP BY g.guide_name
ORDER BY total_assignments DESC;

-- 4. Find High-Rated Guides Handling Premium / Paid Activities
-- Ensure top-rated guides handle paid or premium activities.
SELECT g.guide_name,
       g.rating,
       a.activity_name,
       a.price
...
-- 5. Daily Guide Schedule 
-- Check a guide’s daily schedule to prevent overlapping assignments.
SELECT g.guide_name,
       DATE(ga.start_time) AS tour_date,
       MIN(ga.start_time) AS first_tour,
       MAX(ga.end_time) AS last_tour,
       COUNT(*) AS total_tours
FROM GuideAssignments ga
JOIN TourGuides g ON ga.guide_id = g.guide_id
WHERE ga.status = 'Assigned'
GROUP BY g.guide_name, DATE(ga.start_time)
ORDER BY tour_date, total_tours DESC;
...
-- REAL-WORLD EXAMPLE
-- 1. Top-Rated Destinations Based on Customer Reviews
-- Marketing team wants to promote highest-rated destinations.
SELECT d.destination_name,
       ROUND(AVG(r.rating), 2) AS avg_rating,
       COUNT(r.review_id) AS total_reviews
FROM Reviews r
...
-- 2. Identify Low-Rated Reviews for Service Improvement
-- Customer support checks negative feedback to improve services.
SELECT c.customer_name,
       d.destination_name,
       r.rating,
       r.comments,
...
-- Find popular reviews to feature on website or ads.
SELECT c.customer_name,
       d.destination_name,
       r.rating,
       r.comments,
...
-- 4. Average Rating Per Booking (Trip Quality Check)
-- Operations team evaluates overall trip satisfaction per booking.
SELECT r.booking_id,
       ROUND(AVG(r.rating), 2) AS avg_booking_rating,
       COUNT(r.review_id) AS review_count
FROM Reviews r
WHERE r.booking_id IS NOT NULL
GROUP BY r.booking_id
ORDER BY avg_booking_rating DESC;

-- 5. Monthly Review Trend Analysis
-- Management checks customer satisfaction trends over time.
SELECT 
    YEAR(review_date) AS review_year,
    MONTH(review_date) AS review_month,
    ROUND(AVG(rating), 2) AS avg_rating,
...
-- Finance team tracks monthly revenue trends.
SELECT 
...
-- 2. Most Preferred Payment Methods
-- Business team analyzes customer payment preferences.
SELECT 
...
-- Management identifies premium customers & bookings.
SELECT 
    p.booking_id,
    c.customer_name,
    p.amount,
...
-- 4. Average Payment Amount per Booking
-- Helps understand average booking value.
SELECT 
    booking_id,
...
-- 5. Customers Who Paid Using Multiple Payment Methods
-- Detects repeat customers or split payments behavior.
SELECT 
    c.customer_name,
...
-- REAL-WORLD EXAMPLE
-- 1. Open Complaints Needing Immediate Attention
-- Customer support wants to track pending issues, especially high priority ones.
SELECT 
    complaint_id,
    subject,
    priority,
...
-- Management analyzes which service areas cause the most complaints.
SELECT 
    subject,
    COUNT(*) AS total_complaints
FROM Complaints
...
-- 3. Resolution Rate 
-- Measures how efficiently complaints are resolved.
SELECT
    COUNT(*) AS total_complaints,
...
-- 4. Customers with Repeated Complaints
-- Identifies high-risk or dissatisfied customers for retention actions.
SELECT 
    c.customer_name,
...
-- 5. High-Priority Complaints by Booking
-- Operations team checks which bookings caused critical issues.
SELECT 
    co.booking_id,
    b.booking_date,
    COUNT(co.complaint_id) AS high_priority_issues
FROM Complaints co
JOIN Bookings b ON co.booking_id = b.booking_id
WHERE co.priority = 'High'
GROUP BY co.booking_id, b.booking_date
...
-- REAL-WORLD EXMPLES
-- 1. Active Staff Count by Department
-- Management wants to know department-wise workforce strength.
SELECT 
    department,
    COUNT(*) AS total_staff
FROM Staff
...
-- HR reviews salary distribution and top earners.
SELECT 
    name,
    role,
    department,
...
-- Used for salary benchmarking and appraisal planning.
SELECT 
    role,
...
-- Identifies senior and experienced staff for leadership or critical projects.
SELECT 
    name,
    role,
    department,
...
-- Finance team calculates monthly payroll impact per department.
SELECT 
    department,
...
-- REAL-WORLD EXAMPLES
-- 1. Daily Staff Deployment Report
-- Operations manager checks who is working on a specific date and in what role.
SELECT 
    sa.shift_date,
    s.name,
    sa.role,
...
FROM StaffAssignments sa
JOIN Staff s ON sa.staff_id = s.staff_id
WHERE sa.shift_date = '2025-09-10'
ORDER BY sa.start_time;

-- 2. Staff Workload Count 
-- HR identifies overloaded vs underutilized employees.
SELECT 
    s.name,
    s.department,
...
-- 3. Booking-wise Staff Allocation
-- Tour coordinator checks which staff are assigned to each booking.
SELECT 
    sa.booking_id,
    s.name,
    sa.role,
...
-- 4. Role-wise Shift Coverage Analysis
-- Management ensures enough staff per role (Managers, Guides, Drivers).
SELECT 
    role,
    COUNT(*) AS total_shifts
FROM StaffAssignments
...
-- 5. Staff Working More Than 8 Hours in a Shift
-- HR checks overtime or compliance with labor policies.
SELECT 
    s.name,
    sa.shift_date,
    sa.start_time,
    sa.end_time,
    TIMESTAMPDIFF(HOUR,
        CONCAT(sa.shift_date,' ',sa.start_time),
        CONCAT(sa.shift_date,' ',sa.end_time)
    ) AS working_hours
FROM StaffAssignments sa
JOIN Staff s ON sa.staff_id = s.staff_id
